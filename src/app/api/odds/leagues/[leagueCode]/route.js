...
import { isDataAccessTimeoutError } from "../../../../../lib/data-access";
...
export async function GET(request, { params }) {
  try {
    ...
    return NextResponse.json({
      ...
      degraded: Boolean(league.degraded),
    ...
    const status = isDataAccessTimeoutError(error) ? 503 : 500;

    return NextResponse.json(
      {
        error: isDataAccessTimeoutError(error)
          ? "League data is temporarily unavailable."
          : "Failed to load league odds.",
      },
      { status }
    );
  }
